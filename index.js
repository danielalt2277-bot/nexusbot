...
function startVouching(userId, channelId, targetUserId) {
    ...
    const run = async () => {
        ...
        try {
            ...
            const delay = Math.floor(Math.random() * (180000 - 120000 + 1)) + 120000; // 2-3 mins
...
const commands = [
    ...
    { name: 'deleteall', description: 'Deletes all keys and stops all services.' }
];
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isCommand()) {
        ...
        if (commandName === 'auto-bump') {
            ...
            if (userId !== '1439904639490199682') return interaction.reply({ content: 'Unauthorized.', flags: 64 });
            ...
            if (userId !== '1439904639490199682') {
                return interaction.reply({ content: 'Unauthorized.', flags: 64 });
            }

            // Stop all services
            db.data.users.forEach(user => {
                if (user.services?.autoBump?.isActive) {
                    stopBumping(user.id);
                    user.services.autoBump.isActive = false;
                }
                if (user.services?.autoVouch?.isActive) {
                    stopVouching(user.id);
                    user.services.autoVouch.isActive = false;
                }
                if (user.services?.autotrade?.isActive) {
                    stopTrading(user.id);
                    user.services.autotrade.isActive = false;
                }
                if (user.services?.autochat?.isActive) {
                    stopChatting(user.id);
                    user.services.autochat.isActive = false;
                }
            });
            ...
            await db.write();
            await interaction.reply({ content: 'Successfully deleted all keys and stopped all running services.', flags: 64 });
        }
